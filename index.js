...
function findEndpoints(html, scripts) {
  const map = new Map();
  const text = [html, ...scripts.map((s) => (s.includes(':') && s.startsWith('inline:') ? s.slice(s.indexOf(':') + 1) : s))].join('\n');
...
async function fetchVia(url) {
  // direct first; fall back to a CORS proxy (Vercel functions have no CORS restriction)
...
exports.api_scan = async function handler(req, res) {
  ...
  let result, source = 'gemini';
  try {
    result = await callGemini(buildPrompt(target, html, endpoints));
    if (!result.target) throw new Error('missing target');
    result.worker_code = result.worker_code || FALLBACK_WORKER;
  } catch (e) {
    source = 'fallback';
    const best = endpoints[0];
    const guessUrl = best && !/^https?:/i.test(best.url) ? new URL(best.url, target).href : best ? best.url : target;
    result = {
      target: guessUrl,
      method: 'POST',
  ...
  res.json({
    ...
    analysis: result,
    source,
    api_error: source === 'fallback' ? 'see server logs' : null,
  });
};
